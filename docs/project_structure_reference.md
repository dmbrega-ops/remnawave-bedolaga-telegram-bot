...
- `app/cabinet/services/email_service.py` — Python-модуль
  Классы: `EmailService` (28 методов)
  Функции: нет
...
- `app/cabinet/services/email_templates.py` — Python-модуль
  Классы: `EmailNotificationTemplates` (49 методов)
  Функции: нет
...
- `app/services/web_auth_service.py` — Python-модуль
  Классы: нет
  Функции: `create_web_auth_token` — Generate a web auth token and store it in Redis (pending state)., `create_magic_link_token` — Generate a magic link (passwordless login) token and store it in Redis., `link_web_auth_token` — Link a web auth token to a Telegram user (called by bot on /start)., `poll_web_auth_token` — Poll for web auth token status (non-destructive)., `consume_web_auth_token` — Atomically get and delete a web auth token.
